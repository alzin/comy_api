...
export class CheckSubscriptionStatusController {
  constructor(private checkSubscriptionStatusUseCase: CheckSubscriptionStatusUseCase) {}

  async handle(req: Request, res: Response): Promise<void> {
    try {
      const isActive = await this.checkSubscriptionStatusUseCase.execute(req.user.id);
      res.json({ isActive });
    } catch (error) {
