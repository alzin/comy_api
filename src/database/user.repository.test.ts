...
describe('UserRepository', () => {
  ...
  describe('findByVerificationToken', () => {
    it('should return null if user not found', async () => {
      const user = await userRepository.findByVerificationToken('nonexistent-token');
      expect(user).toBeNull();
    });
    ...
    it('should return user if found', async () => {
      ...
      const foundUser = await userRepository.findByVerificationToken('valid-token');
      expect(foundUser).toEqual(newUser);
    });
  });
  ...
  describe('update', () => {
    ...
    it('should throw an error if user does not exist', async () => {
      ...
      await expect(userRepository.update(nonExistentUser)).rejects.toThrow('User not found');
    });
  });
});